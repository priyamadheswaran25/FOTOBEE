...
export const create = async (req: Request, res: Response) => {
    try {
        ...
        const data = await prisma.testimonial.create({ data: parsed.data as any as any });
...
export const update = async (req: Request, res: Response) => {
    try {
        const parsed = testimonialSchema.safeParse(req.body);
        if (!parsed.success) return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid data', details: parsed.error } });
        const data = await prisma.testimonial.update({ where: { id: req.params.id }, data: parsed.data as any as any });
        res.json({ success: true, data });
    } catch (error) {
